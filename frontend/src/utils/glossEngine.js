...
// ✅ Clean, single declaration for Vite environment variables
const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:5000';

const STOP = new Set(['A','AN','THE','IS','AM','ARE','WAS','WERE','BE','TO','OF','AND','OR','BUT','SO','IF','THAT','THIS','IT','IN','ON','AT','FOR','WITH']);
const LOCAL = { 
  HELLO:'HELLO', HI:'HELLO', YOU:'YOU', YOUR:'YOUR', ME:'ME', MY:'MY', NAME:'NAME',
...
function legacyToTokens(data) {
  ...
  const out = [];
  if (typeof d.gloss === 'string') {
    for (const w of d.gloss.split(/\s+/)) { 
      const n = norm(w); 
      if (n) out.push({ type: 'gloss', value: n }); 
    }
  }
  for (const u of unknown) {
    if (u) out.push({ type: 'spell', letters: u.split('') });
  }
