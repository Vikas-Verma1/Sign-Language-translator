...
/* === VISUAL ONLY (Step 1): studio environment for realistic PBR skin === */
...
const RPM_POSES = {
  ...
  ME: {
    arm: { RightArm: [-0.6, 0.5, 0.2], RightForeArm: [-1.2, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.POINT,
  },
  THANKS: {
    arm: { RightArm: [-1.1, 0, 0.3], RightForeArm: [-0.8, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.OPEN_PALM,
  },
  WHAT: {
  ...
  WHERE: {
    arm: { RightArm: [-1.0, 0.2, 0.6], RightForeArm: [-0.6, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.POINT,
  },
  NAME: {
  ...
  PLEASE: {
    arm: { RightArm: [-0.5, 0.3, 0.2], RightForeArm: [-1.2, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.OPEN_PALM,
  },
  YES: {
    arm: { RightArm: [-0.9, 0, 0.5], RightForeArm: [-0.7, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.FIST,
  },
  NO: {
    arm: { RightArm: [-1.0, 0, 0.4], RightForeArm: [-0.5, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
    hand: FINGER_SHAPES.POINT,
  }
};

const FALLBACK_POSE = {
  arm: { RightArm: [-0.9, 0.2, 0.5], RightForeArm: [-0.8, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
  hand: FINGER_SHAPES.OPEN_PALM,
};
  ...
  scene.traverse((child) => {
    if (child.isBone && !matchedBone) {
      ...
      for (const kw of keywords) {
        if (name.includes(kw.toLowerCase())) { matchedBone = child; break; }
      }
    }
  });
  return matchedBone;
...
export default function AvatarCanvas({ currentGloss }) {
  ...
  useEffect(() => {
    ...
    /* === VISUAL ONLY (Step 1): transparent so the CSS gradient shows through === */
    scene.background = null;

    const camera = new THREE.PerspectiveCamera(45, width / height, 0.1, 1000);
    /* === VISUAL ONLY (Step 1): distance tuned to show the UPPER body === */
    camera.position.set(0, 0.05, 1.9);
    camera.lookAt(0, 0, 0);
    ...
    /* === VISUAL ONLY (Step 1): filmic tone mapping for believable skin === */
    renderer.toneMapping = THREE.ACESFilmicToneMapping;
    renderer.toneMappingExposure = 1.05;
    mountRef.current.appendChild(renderer.domElement);

    /* === VISUAL ONLY (Step 1): studio environment + 3-point lighting === */
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        /* === VISUAL ONLY (Step 1): frame the UPPER body for ANY rig ===
           We align ~68% of the model's height (chest/face) to world Y = 0 and
           keep natural proportions (no shrink) so realistic rigs look correct.
           Tweak the two numbers below (0.68 and camera z above) if you want a
           tighter/looser crop. This is presentation only — no logic touched. */
        const box = new THREE.Box3().setFromObject(avatar);
        const size = box.getSize(new THREE.Vector3());
        const center = box.getCenter(new THREE.Vector3());
        const focusY = box.min.y + size.y * 0.68;
        avatar.position.set(-center.x, -focusY, -center.z);

        /* === CORE LOGIC — UNCHANGED: bone keyword mapping === */
        const detectedBones = {
          RightArm: findBone(avatar, ['rightarm', 'armr', 'upperarmr', 'shoulderr', 'mixamorigrightarm']),
          RightForeArm: findBone(avatar, ['rightforearm', 'forearmr', 'lowerarmr', 'elbowr', 'mixamorigrightforearm']),
          LeftArm: findBone(avatar, ['leftarm', 'arml', 'upperarml', 'shoulderl', 'mixamorigleftarm']),
          LeftForeArm: findBone(avatar, ['leftforearm', 'forearml', 'lowerarml', 'elbowl', 'mixamorigleftforearm']),
  ...
  useEffect(() => {
    ...
    const normalizedGloss = currentGloss.trim().toUpperCase();
    if (normalizedGloss === 'REST') {
      targetPose.current = RPM_POSES.REST;
    } else {
      targetPose.current = RPM_POSES[normalizedGloss] || FALLBACK_POSE;
    }
  }, [currentGloss]);
  /* =================== /CORE LOGIC — UNCHANGED =================== */
...
//   ME: {
//     arm: { RightArm: [-0.6, 0.5, 0.2], RightForeArm: [-1.2, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
//     hand: FINGER_SHAPES.POINT, // Pointing finger towards chest
//   },
//   THANKS: {
//     arm: { RightArm: [-1.1, 0, 0.3], RightForeArm: [-0.8, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
//     hand: FINGER_SHAPES.OPEN_PALM, // Fingers moving from chin outward
//   },
//   WHAT: {
//     arm: { RightArm: [-0.7, 0.4, 0.5], RightForeArm: [-0.8, 0, 0], LeftArm: [-0.7, -0.4, -0.5], LeftForeArm: [-0.8, 0, 0] },
...
//   WHERE: {
//     arm: { RightArm: [-1.0, 0.2, 0.6], RightForeArm: [-0.6, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
//     hand: FINGER_SHAPES.POINT, // Index finger up shaking
//   },
//   NAME: {
//     arm: { RightArm: [-0.9, 0.2, 0.3], RightForeArm: [-1.1, 0, 0], LeftArm: [-0.9, -0.2, -0.3], LeftForeArm: [-1.1, 0, 0] },
...
// // Fallback sign pose for words not explicitly defined in dictionary
// const FALLBACK_POSE = {
//   arm: { RightArm: [-0.9, 0.2, 0.5], RightForeArm: [-0.8, 0, 0], LeftArm: [0.2, 0, -0.8], LeftForeArm: [0, 0, 0] },
//   hand: FINGER_SHAPES.OPEN_PALM,
// };
...
//         // Map Arm and Finger Bones with enhanced keyword matching
//         const detectedBones = {
//           RightArm: findBone(avatar, ['rightarm', 'armr', 'upperarmr', 'shoulderr', 'mixamorigrightarm']),
//           RightForeArm: findBone(avatar, ['rightforearm', 'forearmr', 'lowerarmr', 'elbowr', 'mixamorigrightforearm']),
//           LeftArm: findBone(avatar, ['leftarm', 'arml', 'upperarml', 'shoulderl', 'mixamorigleftarm']),
...
//       if (bonesRef.current) {
//         // 1. Interpolate Arm Rotations smoothly
//         const armTargets = targetPose.current.arm || RPM_POSES.REST.arm;
//         Object.keys(armTargets).forEach((key) => {
//           const bone = bonesRef.current[key];
//           if (bone) {
...
//   // Update target pose when currentGloss changes
//   useEffect(() => {
//     if (!currentGloss) return;
//     const normalizedGloss = currentGloss.trim().toUpperCase();
    
//     // Match exact pose dictionary or trigger generic gesture fallback
//     if (normalizedGloss === 'REST') {
//       targetPose.current = RPM_POSES.REST;
//     } else {
//       targetPose.current = RPM_POSES[normalizedGloss] || FALLBACK_POSE;
